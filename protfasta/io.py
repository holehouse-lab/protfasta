...
def check_inputs(expect_unique_header, 
    ...
    # first validation of header_parser
    # always check that 'header_parser' is a function to prevent unexpected errors incase non-function objects are passed
    ...
    # second validation of header_parser if 'check_header_parser' is true
...
def internal_parse_fasta_file(filename, expect_unique_header=True, header_parser=None, verbose=False):
    ...
    try:
        filepath.open(mode="r")
    except FileNotFoundError:
        raise ProtfastaException('Unable to find file: %s'%(filename))
    
    if verbose:

        # fast and lightweight line counter
        num_lines = sum(1 for _ in filepath.open(mode="r"))
          
        print('[INFO]: Read in file with %i lines'%(num_lines))

    # note, we'll keep the ability to directly parse dictionaries
    return _parse_fasta_all(filepath, expect_unique_header=expect_unique_header, header_parser=header_parser, verbose=verbose)
...
#
#    
def _parse_fasta_all(filepath, expect_unique_header=True, header_parser=None, verbose=False):
    """
    ...
    -----------
    filepath : Path
        A Path object generated by reading in a file name.
    ...
    # using Path's built-in open method allows this function to open
    # arbitrarily sized fasta files without reading entirely into memory
     
    content = filepath.open(mode="r")
